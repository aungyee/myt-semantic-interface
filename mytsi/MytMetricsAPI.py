...
class MyTMetricsAPI:
    ...
    def _get_catalog(self) -> None:
        ...
        self.tables = pd.DataFrame(requests.get(url=endpoint, headers=headers).json()['results']).sort_values('name', ascending=True).name.values
        ...
        for i in tqdm(range(self.num_catalog_to_load)):
            ...
            if response['status'] == 'ok':
                ...
                results = {
                        ...
                        "joined_catalogs": [response_results["joinedTables"] for _ in range(len(response_results["fields"]))]
                    }
    ...
    def __init__(self, base_url: str, personal_access_token: str, project_uuid: str, num_catalog_to_load: int = None) -> None:
    ...
    def get_all_metrics(self) -> list:
        ...
        Get all metrics
        """
        return self.fields[self.fields.field_fieldType == 'metric'][["field_name", "field_basicType", "field_description", "catalog_name"]]
    ...
    def get_available_dimensions_for_metric(self, metric_name: str) -> list:
        ...
        return self.fields[(self.fields.field_fieldType == 'dimension') & (self.fields.catalog_name.isin(metric_of_interest["joined_catalogs"]))][["field_name", "field_basicType", "field_description", "catalog_name"]]
    ...
    def prepare_metrics_for_langchain(self) -> pd.DataFrame:
        ...
        metrics['available_dimensions'] = metrics.metric_name.apply(lambda x: self.get_available_dimensions_for_metric(metric_name=x).to_dict(orient='records'))
        metrics['available_dimensions'] = metrics['available_dimensions'].astype(str)
        return metrics

    def get_metrics_query(self, metrics: list, dimensions: list = [], filters: dict = {}, table_calculation: list = [], sorts: list = [], additional_metrics: list = [], limit: int = 1):
        """
        ...
        final_metric_names = list(metrics_of_interest.apply(lambda x: f"{x.catalog_name}_{x.field_name}", axis=1).values)
        ...
        final_dimension_names = list(dimensions_of_interest.apply(lambda x: f"{x.catalog_name}_{x.field_name}", axis=1).values)
